...
class AgentMFPMultiSameTransProb(Agent) :
	...
	def __init__(
		...
		self.alphas = fixed_parameters["alphas"]
	...
	def payoff(self, action, partners):		
		if action == 0:
			utility = 0
		elif sum(partners) <= self.treshold*self.num_agents - 1:
			utility = 1
		else:
			utility = -1
		if self.debug:
			print(f'Action:{action} --- sum partners:{sum(partners)} --- capacity:{self.threshold*self.num_agents} --- utility:{utility}')
		return utility
...
class softmax_greedy(AgentMFPMultiSameTransProb):

	def __init__(self, fixed_parameters: Dict[str, Any], free_parameters: Dict[str, Any], n: int) -> None:
		super().__init__(fixed_parameters, free_parameters, n)
	...
	def determine_action_preferences(self, prev_state:any) -> List[float]:
		# To be defined by subclass
		expected_utilities = [self.exp_util(prev_state, action) for action in range(2)]
		if self.debug:
			print(f'Expected utilities: {expected_utilities}')
